...
export const sectionize_notes = (notes: INoteData[]) => {
    ...
    const aWeekTS = 1000 * 60 * 60 * 24 * 7;
    const aDayAgoTS = 1000 * 60 * 60 * 24;
    const twoDaysAgoTS = 1000 * 60 * 60 * 24 * 2;
    const aDayAgo = new Date(Date.now() - aDayAgoTS).getTime();
    const twoDaysAgo = new Date(Date.now() - twoDaysAgoTS).getTime();

    console.log("parsed_notes", parsed_notes, notes)

    const todays_notes = parsed_notes.filter(note => (note.note as TNote).time > aDayAgo || Object.keys((note.note as TNote)).length == 0);
    const yesterday_notes = parsed_notes.filter(note => (note.note as TNote).time < aDayAgo && (note.note as TNote).time > twoDaysAgo);
    const previous_notes = parsed_notes.filter(note => (note.note as TNote).time < twoDaysAgo)
    ...
    if (yesterday_notes.length > 0) {
        sections['yesterday'] = yesterday_notes;
    }
